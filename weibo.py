...
class Weibo(object):
                ...
                if w[key]:
                    ...
                    if type == 'img' and ',' in w[key]:
                        ...
                        for j, url in enumerate(w[key]):
                            ...
                            self.download_one_file(url, file_path, type,w['id'])
                    else:
                        if type == 'video':
                            file_suffix = '.mp4'
                        else:
                        ...
                        file_path = file_dir + os.sep + file_name
                        self.download_one_file(w[key], file_path, type,w['id'])
    ...
    def get_result_headers(self):
        ...
        if not self.filter:
            result_headers2 = ['是否原创', '原用户id', '原用户昵称']
            result_headers3 = ['原微博' + r for r in result_headers]
    ...
    def get_pages(self):
        ...
        for page in tqdm(range(1, page_count + 1), desc=u"进度"):
            ...
            if page % 20 == 0:  # 每爬20页写入一次文件
                ...
                '''通过加入随机等待避免被限制。爬虫速度过快容易被系统限制(一段时间后限制会自动解除)，
...
def main():
    ...
    try:
        ...
        i = input()
        user_id_list = [i]
        print("输入起始时间，即爬取发布日期从该值到现在的微博，形式为yyyy-mm-dd：")
        a = input()
        since_date = a
        print("0表示爬取全部微博（原创微博+转发微博），1表示只爬取原创微博：")
        b = int(input())
        filter = b
        print("0代表不下载微博原始图片,1代表下载微博原始图片：")
        c = int(input())
        pic_download = c
        print("0代表不下载微博视频,1代表下载微博视频：")
        d = int(input())
        video_download = d
